...
#Control de errores:
#Puede que no se haya ejecutado ninguna vez copia de seguridad, por lo que el archivo binchecker no existe
if [[ ! -d "$dirChanges" ]]
then
...
done >> $dirChanges"/""changes_""$snapshotNow" #El registro demlos cambios se desvía a la ruta actual.
